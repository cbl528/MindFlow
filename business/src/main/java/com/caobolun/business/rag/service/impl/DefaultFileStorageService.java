...
import com.caobolun.business.rag.config.RagStorageProperties;
import com.caobolun.business.rag.core.storage.ObjectStorageClient;
 ...
 * 与 {@link StoredFileDTO} 装配，底层裸操作委托给 {@link ObjectStorageClient}（MinIO / rustfs，由
 * {@code rag.storage.type=s3} 决定）；存储引用只保留裸 key，桶是部署级配置常量、不写进数据，改桶名不会令
 * 历史引用失联
 */
@Slf4j
@Service
public class DefaultFileStorageService implements FileStorageService {
    ...
    private static final String LOCK_KEY_PREFIX = "mindflow:kb:space:init:";
